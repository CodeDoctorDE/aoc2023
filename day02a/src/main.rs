...
fn is_possible(game: &Game, red: u8, green: u8, blue: u8) -> bool {
    red >= game.red && green >= game.green && blue >= game.blue
}

fn count_indexes(games: Vec<Game>, red: u8, green: u8, blue: u8) -> u32 {
    let mut count = 0_u32;
    for game in games {
        if is_possible(&game, red, green, blue) {
            count += game.id as u32;
        }
    }
    count
}
...
fn algo(input: String) -> u32 {
    ...
    count_indexes(games, RED_MAX, GREEN_MAX, BLUE_MAX)
}
...
mod tests {
    ...
    fn test_example() {
        ...
        assert_eq!(algo(input.to_string()), 8);
    }
}
